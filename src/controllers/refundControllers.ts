...
export const claimRefund = async (req: any, res: Response) => {
    try {
        ...
        const refundPlans = refundableCartItems.map((item) => {
            const unitPrice = Number(item.plan?.price ?? 0);
            const totalQty = Number(item.quantity ?? 0);

            // Count successful eSIMs
            const esimCreated = item.esims?.filter(
                (e) => e.iccid && e.qrCodeUrl
            ).length ?? 0;

            // Determine refundable quantity
            const refundableQty =
                order.status === ORDER_STATUS.FAILED
                    ? totalQty
                    : Math.max(totalQty - esimCreated, 0);

            // Final refundable amount
            const refundableAmount = refundableQty * unitPrice;
