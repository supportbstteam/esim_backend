...
export const createContacts = async (req: Request, res: Response) => {
  try {
    const contacts = req.body; // expects an array of contacts
    if (!Array.isArray(contacts) || contacts.length === 0) {
      return res.status(400).json({ message: "Contacts array is required" });
    }

    const newContacts = contactRepo.create(contacts);
    await contactRepo.save(newContacts);

    return res.status(201).json(newContacts);
  } catch (err) {
    return res.status(500).json({ message: "Error creating contacts", error: err });
  }
};
